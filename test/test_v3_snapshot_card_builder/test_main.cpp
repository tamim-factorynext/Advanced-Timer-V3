...
void test_build_runtime_snapshot_card_for_do_uses_typed_runtime_state() {
  ...
  dOut[0].commandState = true;
  dOut[0].actualState = true;
  dOut[0].edgePulse = true;
  ...
  TEST_ASSERT_TRUE(snap.commandState);
  TEST_ASSERT_TRUE(snap.actualState);
  TEST_ASSERT_TRUE(snap.edgePulse);
  TEST_ASSERT_EQUAL(State_DO_Active, snap.state);
  TEST_ASSERT_EQUAL_UINT32(1, snap.liveValue);
...
void test_build_runtime_snapshot_cards_for_mixed_families() {
  ...
  rtc[0].commandState = true;
  rtc[0].edgePulse = true;
  rtc[0].liveValue = 1;
  ...
  TEST_ASSERT_EQUAL_UINT32(2345, out[0].liveValue);
  ...
  TEST_ASSERT_TRUE(out[1].commandState);
  TEST_ASSERT_TRUE(out[1].edgePulse);
  TEST_ASSERT_EQUAL_UINT32(1, out[1].liveValue);
