...
void test_do_runtime_fields_are_cleared_from_config_card() {
  ...
  TEST_ASSERT_FALSE(card.commandState);
  TEST_ASSERT_FALSE(card.actualState);
  TEST_ASSERT_FALSE(card.edgePulse);
  TEST_ASSERT_EQUAL_UINT32(0, card.liveValue);
...
void test_array_sanitize_applies_to_all_cards() {
  ...
  TEST_ASSERT_EQUAL_UINT32(0, cards[0].liveValue);
  TEST_ASSERT_EQUAL(State_DI_Idle, cards[0].state);
  TEST_ASSERT_EQUAL_UINT32(0, cards[1].liveValue);
