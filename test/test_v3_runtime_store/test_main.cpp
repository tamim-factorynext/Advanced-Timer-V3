...
void test_sync_runtime_store_from_typed_cards_copies_family_state() {
  LogicCard cards[3] = {};
  cards[0].index = 0;
  cards[0].commandState = true;
  cards[1].index = 0;
  cards[1].liveValue = 321;
  cards[1].state = State_AI_Streaming;
  cards[2].index = 0;
  cards[2].edgePulse = true;
  ...
  TEST_ASSERT_TRUE(di[0].commandState);
  TEST_ASSERT_EQUAL_UINT32(321, ai[0].liveValue);
  TEST_ASSERT_EQUAL(State_AI_Streaming, ai[0].state);
  TEST_ASSERT_TRUE(rtc[0].edgePulse);
...
void test_mirror_runtime_store_card_to_legacy_by_typed_updates_card() {
  ...
  TEST_ASSERT_EQUAL_UINT32(5, card.liveValue);
